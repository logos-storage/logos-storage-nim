...
int download_init(StorageCtx *storage_ctx, const char *cid)
{
    ...
    if (storage_ctx_download_init(storage_ctx, nimffi_str(cid), chunk_size, true, true, on_str_reply, r) != RET_OK)
    {
        free_resp(r);
        return RET_ERR;
    }
...
int check_download_manifest(StorageCtx *storage_ctx, const char *cid)
{
    ...
    if (storage_ctx_download_manifest(storage_ctx, nimffi_str(cid), true, on_str_reply, r) != RET_OK)
    {
        free_resp(r);
        return RET_ERR;
    }
